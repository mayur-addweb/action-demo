...
/**
 * Returns the price based on the event price fields and current date.
 */
class EventPriceResolver implements PriceResolverInterface {
  ...
  public function resolve(PurchasableEntityInterface $entity, $quantity, Context $context) {
    ...
    if (!$pricing_options = $this->priceManager->getEventPricingOptions($entity, $quantity, $context)) {
      return NULL;
    }
    ...
    if (($customer instanceof UserInterface) && ($this->membershipChecker->isMemberInGoodStanding($customer) || $customer->hasRole('vscpa_administrator'))) {
      return $pricing_options['current_option']['member_price'];
    }
    else {
      return $pricing_options['current_option']['price'];
    }
  }
