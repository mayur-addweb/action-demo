 ...
 */
class CpeReceiptController extends ControllerBase {
  ...
  public function download($id = NULL, $event_code = NULL, $event_year = NULL) {
    ...
    try {
      $response = $this->getReceiptResponse($id, $event_code, $event_year);
    }
    ...
    if ($receipt_not_available) {
      ...
      $message = $this->t('The Receipt related to the event <strong>@event-code/@event-year</strong> is not available. Your Receipt will be ready within 3 weeks of the event.', $message_params);
    ...
    $filename = 'cpe-Receipt' . $id . '-' . $event_code . '-' . $event_year . '.pdf';
   ...
   */
  public function getReceiptResponse($id = NULL, $event_code = NULL, $event_year = NULL) {
    $params = [
      'id' => $id,
    ...
    return $this->client->get('/report/events/confirmation', $params, 'raw');
  }
